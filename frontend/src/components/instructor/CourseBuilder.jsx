...
const CourseBuilder = () => {
  ...
  const initialState = {
    ...
    quizzes: [ // 🚩 Array for Multiple Quizzes
      { 
  ...
  const addQuiz = () => {
    setCourse({ 
      ...course, 
      quizzes: [...course.quizzes, { quiz_title: "", passing_marks: 1, questions: [] }] 
    });
  };
  ...
  const handlePublish = async () => {
    if (!course.title || course.lessons.length === 0) return alert("Fill mandatory fields! ❌");
    try {
      await API.post("/api/instructor/create-course", course);
      alert("Course Published Successfully! 🚀");
      setCourse(initialState); // 🚩 Clear all fields
    ...
    <div className="min-h-screen bg-white font-sans text-slate-900">
      ...
      <header className="sticky top-0 z-50 bg-slate-900 text-white p-4 px-8 flex justify-between items-center border-b-2 border-blue-600">
        <div className="flex items-center gap-6">
          ...
          <h1 className="text-xl font-black italic uppercase tracking-tighter">STUDIO<span className="text-blue-500">.</span>BUILDER</h1>
        </div>
        
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-3 border-l-2 border-slate-700 pl-6">
            <div className="text-right hidden sm:block">
              <p className="text-[9px] font-black uppercase text-blue-400 tracking-widest">Active Instructor</p>
          ...
          <button onClick={handlePublish} className="bg-white text-slate-900 hover:bg-blue-500 hover:text-white px-6 py-2 rounded-lg font-black text-xs transition-all uppercase shadow-lg shadow-white/10">
            Publish Content
          </button>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto p-6 md:p-12">
        ...
        <section className="mb-16 border-l-4 border-blue-600 pl-8">
          <h3 className="text-sm font-black uppercase tracking-widest mb-8 text-slate-400">01. Course Blueprint</h3>
        ...
        <section className="mb-16">
          <div className="flex justify-between items-center mb-8 border-b-2 border-slate-100 pb-4">
            <h3 className="text-sm font-black uppercase tracking-widest text-slate-400">02. Curriculum (Videos)</h3>
          ...
          <div className="space-y-4">
            {course.lessons.map((l, i) => (
              <div key={i} className="bg-white border-2 border-slate-900 p-6 rounded-2xl flex flex-wrap md:flex-nowrap gap-4 items-end relative shadow-[6px_6px_0px_0px_rgba(15,23,42,0.1)]">
                ...
                <button onClick={() => setCourse({...course, lessons: course.lessons.filter((_, idx) => idx !== i)})} className="p-3 text-red-500 hover:bg-red-50 rounded-xl transition"><Trash2 size={20}/></button>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-20">
           <div className="flex justify-between items-center mb-8 border-b-2 border-slate-100 pb-4">
            <h3 className="text-sm font-black uppercase tracking-widest text-slate-400">03. Assessments (Quizzes)</h3>
            <button onClick={addQuiz} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-[10px] font-black uppercase flex items-center gap-2"><Plus size={14}/> New Quiz</button>
          </div>
              ...
              <div key={qIdx} className="bg-slate-900 p-8 rounded-[3rem] text-white">
                <div className="flex justify-between items-center mb-8">
                  <div className="flex-1 max-w-md">
                    <input className="bg-transparent border-b-2 border-blue-500 text-xl font-black italic outline-none w-full pb-2" placeholder="Quiz Title (e.g. Module 1 Test)" value={q.quiz_title} onChange={e => {
                        const n = [...course.quizzes]; n[qIdx].quiz_title = e.target.value; setCourse({...course, quizzes: n});
                    }}/>
                  </div>
                  <button onClick={() => setCourse({...course, quizzes: course.quizzes.filter((_, i) => i !== qIdx)})} className="text-red-400"><XCircle/></button>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {q.questions.map((qst, qstIdx) => (
                    <div key={qstIdx} className="bg-white text-slate-900 p-6 rounded-3xl space-y-4">
                      <input className="w-full border-b-2 border-slate-100 font-bold text-sm pb-2 outline-none focus:border-blue-500" placeholder="Question text?" value={qst.text} onChange={e => {
                      ...
                      <div className="space-y-2">
                        {qst.options.map((opt, oIdx) => (
                          <div key={oIdx} className="flex items-center gap-2">
                            <input className={`flex-1 p-2 rounded-lg text-[11px] font-bold border-2 ${opt.is_correct ? 'border-green-500 bg-green-50' : 'border-slate-100'}`} placeholder={`Option ${oIdx+1}`} value={opt.text} onChange={e => {
                  ...
                  <button onClick={() => addQuestion(qIdx)} className="border-2 border-dashed border-slate-700 rounded-3xl flex items-center justify-center text-slate-500 font-black text-xs hover:border-blue-500 hover:text-blue-500 transition-all">+ Add Question</button>
                </div>
              </div>
            ))}
          </div>
